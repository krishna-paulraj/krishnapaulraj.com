...
function SeriesDashTailOverlayImpl({
  ...
  // An exact version (binary-searching `getPointAtLength` for the length at
  // `dashStartX`) cost ~40 ms per series on a 365-point bezier — for charts
  // with ~10 series that synchronously blocks the main thread for ~400 ms on
  // the post-measurement re-render, swallowing the first second of the
  // entrance animation.
  //
