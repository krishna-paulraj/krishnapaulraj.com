...
export function Metrics01({ data }: { data: InsightsData }) {
  return (
    <div className="mx-auto w-full max-w-2xl px-6 pb-6 font-sans">
      <div className="mt-5 w-full border-t pt-5">
        ...
        {data.series.some((d) => d.unique_visitors > 0 || d.total_sessions > 0) ? (
          <LineChart
            className="mt-4"
            data={data.series}
          ...
          <div className="mt-4 grid aspect-3/1 w-full place-content-center">
            <p className="text-sm text-muted-foreground">No data yet — check back after some visits.</p>
          </div>
        )}
      </div>
    </div>
